...
@Date:   2016-09-15T17:53:07+08:00
@Email:  20108847@qq.com
@Project: my Projece
@Last modified by:   sundx
@Last modified time: 2016-09-18T14:07:20+08:00
-->



## 基于webpack, express 的前端工程化开发（粉丝经济项目）

我们遇到的新问题：

- webpack如何自动发现entry文件及进行相应的模板配置
- 如何直接处理后端模板的样式、脚本自动引入问题
- 使用node架构后端服务，express创建路由
- 后端只给数据接口，前端拿到接口，直接打印出页面
- 如果使用mongoDB，完全可以抛开php，哈哈
...
- website
    ...
    README.md				#项目说明
```
> 你同样可以根据个人喜好自由设计目录结构。完整的源码前往<ssh://git@git.intra.weibo.com:2222/dexin3/fans-economy-fe.git>。
...
```
需要特别说明的是glob依赖，它是一个根据模式匹配获取文件列表的node模块。有关glob的详细用法可以在这里看到——<https://github.com/isaacs/node-glob>。利用glob模块可以很方便的获取src/scripts/page路径下的所有js入口文件。同理，可以实现自动的进行与入口文件相对应的模板配置。

最终的webpack配置如下（不管什么代码，合理的注释会让接棒的队友节约很多时间，这是血淋淋的经验啊，最终这里没有贴出mongoDb的依赖的文件，等以后用到的时候）：
